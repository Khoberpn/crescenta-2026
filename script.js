...
/* ── DEVICE DETECTION (dipakai untuk menekan beban animasi di HP) ── */
...
function createStars() {
  ...
  // Kurangi jumlah bintang drastis di HP agar tidak lag
  const starCount = isLowPowerDevice() ? 24 : 120; 
  ...
  const iconCount = isLowPowerDevice() ? 6 : 25;
...
function createPurpleParticles() {
  ...
  if (!container) return;
  const frag = document.createDocumentFragment();
  const particleCount = isLowPowerDevice() ? 0 : 18;
...
function buildPixelRain() {
  ...
  // ✅ FIX: pixel-rain murni dekoratif & terus-menerus animasi -> berat di HP, skip saja
  if (isLowPowerDevice()) return;

  const cols = Math.min(Math.floor(window.innerWidth / ANIMATION_CONFIG.RAIN_COLUMN_DESKTOP), ANIMATION_CONFIG.RAIN_MAX_COLUMNS);
  const frag = document.createDocumentFragment();
...
function initCityscapeWindows() {
  ...
  // ✅ FIX: interval lebih jarang di HP + auto-pause saat tab tidak aktif
  const speed = isLowPowerDevice() ? 1.8 : 1;
  let intervalIds = [];
  const start = () => {
    intervalIds = [
      setInterval(() => toggleRandomWindow(allWins, 0.05), 800 * speed),
      setInterval(() => toggleRandomWindow(purpleWins, 0.06), 1000 * speed),
      setInterval(() => toggleRandomWindow(pinkWins, 0.06), 1200 * speed)
    ];
  };
  const stop = () => intervalIds.forEach(id => clearInterval(id));
...
/* ── BACKGROUND INTERACTIONS ──
   ✅ FIX PERFORMA: Sebelumnya ada requestAnimationFrame loop yang menimpa
   `transform` di SETIAP FRAME pada 9 elemen dekoratif yang JUGA punya animasi
   CSS sendiri. Dua sumber transform saling berebut/menimpa → inilah penyebab
   tampilan "kacau" sekaligus boros CPU/GPU (lag) di HP. Sekarang cukup
   andalkan animasi CSS (sudah GPU-friendly via translate3d); JS hanya
   menangani interaksi klik (efek koin melayang). */
function initBgInteractions() {
  const bgPac = document.querySelector('.bg-pac-row .pacman');
  const starsLayer = document.getElementById('stars-layer');
  if (!bgPac || !starsLayer) return;
...
function init() {
  try {
    ...
    if (!prefersReducedMotion()) {
      ...
      // Elemen ringan tetap ada di semua device (jumlahnya sudah otomatis
      // dikecilkan lewat isLowPowerDevice() di masing-masing fungsi)
      createStars();
      createPurpleParticles();
      buildPixelRain();
      initBgInteractions();

      // ✅ FIX: sprite dekoratif berat (invader, neon streak, teks ambient)
      // di-skip total di HP — sudah disembunyikan lewat CSS juga, jadi tidak
      // perlu dibuat sama sekali (hemat DOM + memory + animasi GPU)
      if (!lowPower) {
        createInvaders();
        createNeonStreaks();
        createAmbientText();
